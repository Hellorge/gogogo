...
func build() {
	...
	for path, info := range fileInfos {
		trimmedPath := strings.TrimPrefix(path, "server")
		trimmedPath = strings.TrimPrefix(trimmedPath, "/")
		insertIntoBinaryTrie(root, strings.Split(trimmedPath, "/"), &info)
	}

	// Register the FileInfo type with gob
	gob.Register(FileInfo{})

	// Serialize the binary trie
	...
	// Write the binary trie to a file
	if err := os.WriteFile("build_trie.bin", buf.Bytes(), 0644); err != nil {
		fmt.Println("Error writing binary trie:", err)
		os.Exit(1)
	}

	// Optionally, write a human-readable JSON version
	// if *humanReadable {
	//     jsonData, _ := json.MarshalIndent(root, "", "  ")
	//     os.WriteFile("build_trie.json", jsonData, 0644)
	// }
}
